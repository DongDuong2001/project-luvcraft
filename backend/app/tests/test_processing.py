...
def test_is_spam_keywords():
    assert (
        is_spam("This is a FREE GIFT giveaway!", "This is a FREE GIFT giveaway!")
        is True
    )
    assert (
        is_spam("Contact me via whatsapp me now", "Contact me via whatsapp me now")
        is True
    )
    assert (
        is_spam(
            "Legitimate video description about gaming.",
            "Legitimate video description about gaming.",
        )
        is False
    )
...
def test_analyze_sentiment_empty_text_has_no_confidence():
    label, score, confidence = analyze_sentiment("")
    assert label == "neutral"
    assert score == 50.0
...
def test_extract_aspects():
    text = (
        "I love the gameplay and the music of this game! Graphically it is also nice."
    )
