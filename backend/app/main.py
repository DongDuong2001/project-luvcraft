...
async def get_current_user_and_org(db: Session = Depends(get_db)) -> CurrentUser:
    \"\"\"
    Stub dependency for Supabase JWT verification and Org checking.
    Will be replaced with real JWT parsing and DB lookup of organization_members.
    Never trust user ID or organization ID from request body!
    \"\"\"
    # STUB: Return dummy UUIDs until Auth is wired up
    # In reality, verify JWT here, extract user_id, check organization_members table
    return CurrentUser(
...
async def get_historical_runs(
    ...
    current_user: CurrentUser = Depends(get_current_user_and_org)
):
    \"\"\"
    Data Persistence Requirement:
    Persist search runs and results for future review without re-execution.
    Returns previously completed research runs from the PostgreSQL database for the current org.
    \"\"\"
    # Placeholder: fetch from DB using SQLAlchemy ResearchRun models filtering by current_user.organization_id
    return [
        {
            "id": 1,
