...
export default function BrandCollaboration({
  ...
  const displayCollaborations = collaborations.length > 0
    ? collaborations.map((collab, index) => ({
        ...
        color: collab.collaborationScore >= 90 ? 'bg-emerald-500' : 'bg-blue-500',
      }))
    : [
        { id: 1, name: `${keyword} x Gaming Crossover`, match: 92, audiences: 'Overlap: 65%', type: 'Games & Esports', color: 'bg-emerald-500' },
        { id: 2, name: `${keyword} x Digital Media`, match: 86, audiences: 'Overlap: 48%', type: 'Apparel & Media', color: 'bg-blue-500' },
      ];

  const primaryTarget = keyword || 'Target IP';
  const topPartner = displayCollaborations[0]?.name || 'Partner IP';
  const topScore = displayCollaborations[0]?.match || 90;

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-white">Brand-IP Collaboration Match — {primaryTarget}</h2>
          <p className="text-sm text-slate-400 mt-1">Discover high-affinity IP crossovers backed by live signal audience overlap data.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="col-span-1 lg:col-span-2 bg-app-surface border-app-line">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">Visual mapping of audience intersection between {primaryTarget} and target brand profiles.</CardDescription>
          </CardHeader>
          <CardContent className="h-[400px] flex items-center justify-center border border-dashed border-app-line rounded-lg bg-app-bg m-6 mt-0 relative overflow-hidden">
            {/* Dynamic Network Diagram Structure */}
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 border-t-2 border-dashed border-blue-400/50 flex justify-center">
              <div className="bg-app-surface px-2 py-1 -mt-3 text-[10px] font-bold text-white border border-app-line rounded-md flex items-center gap-1">
                <Link className="h-3 w-3 text-blue-400" /> +{topScore} Match
              </div>
            </div>
            
            <p className="absolute bottom-4 left-4 text-xs text-slate-500 flex items-center gap-1">
              <ArrowRightLeft className="h-3 w-3" /> AI Node Force-Directed Graph — Live Pipeline Insights
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="col-span-1 bg-app-surface border-app-line">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">Ranked by backend sentiment score and keyword overlap.</CardDescription>
          </CardHeader>
