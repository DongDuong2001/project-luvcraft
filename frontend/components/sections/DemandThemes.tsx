...
import IntentClusterVisualization from './IntentClusterVisualization';
...
export default function DemandThemes({ data = { status: 'insufficient_data', demands: [], faqs: [], intents: [], themes: [], timeframeStart: null, timeframeEnd: null, methodologyVersion: null }, section = 'all' }: { data?: DemandThemesData; section?: Section }) {
  ...
  return <section className="rounded-xl border border-app-line bg-app-surface p-5 text-slate-200">
    ...
    {showDemand && <><div className="mt-5"><IntentClusterVisualization data={data} /></div><p className="mt-5 text-xs text-slate-500">Method: {data.demandInferenceProvider === 'gemini' ? `${data.demandInferenceModel ?? 'Gemini'} on original-language text` : 'Conservative deterministic fallback'} · LLM: {data.demandLlmClassifiedCount ?? 0} · Fallback: {data.demandFallbackCount ?? 0}</p>{(data.demandWarnings ?? []).map(warning => <p key={warning} className="mt-2 text-xs text-amber-300">{warning}</p>)}</>}
  </section>;
}
