            ...
            <>
              ...
              <div className="mt-5 overflow-x-auto">
                <table className="w-full min-w-[680px] text-left text-sm">
                  ...
                  <thead className="border-b border-app-line text-xs uppercase text-slate-500"><tr><th className="py-2">Source</th><th>Signals</th><th>Positive</th><th>Neutral</th><th>Negative</th><th>Avg score</th><th>Model confidence</th></tr></thead>
                  <tbody className="divide-y divide-app-line">{confidence.sources.map((source) => <tr key={source.source}><th scope="row" className="py-3 font-medium text-slate-200">{source.source}</th><td>{source.usableSignalCount}</td><td className="text-emerald-400">{source.positivePercentage.toFixed(1)}%</td><td>{source.neutralPercentage.toFixed(1)}%</td><td className="text-rose-400">{source.negativePercentage.toFixed(1)}%</td><td>{source.averageSentimentScore.toFixed(1)}</td><td>{Math.round(source.averageModelConfidence * 100)}%</td></tr>)}</tbody>
                </table>
              </div>
