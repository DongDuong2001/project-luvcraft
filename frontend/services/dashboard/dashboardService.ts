...
export interface SourceSentiment { source: string; usableSignalCount: number; positivePercentage: number; neutralPercentage: number; negativePercentage: number; averageSentimentScore: number; averageModelConfidence: number; collectorStatus: string; }
...
export interface CommunityAnalysis { status: string; audienceSegments: AudienceSegment[]; engagementLevel: string | null; discussionDepth: string | null; toxicityLevel: string | null; hospitalityLevel: string | null; consensusLevel: string | null; evidenceSignalIds: string[]; warnings: string[]; }
